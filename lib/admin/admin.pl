...
application_properties(Module) :- current_pengine_application(Module).

show_modules([], []).
show_modules([M-List|T], [Json|JsonList]) :-
  show_module(M, List, Json),
  show_module(T, JsonList).
...
set_settings(Request) :-
  ...
  (
    var(Error) -> catch(set_setting(Module:Setting, Value), Error, true),
    (
      var(Error) -> setting_property(Module:Setting, default(Default)), reply_json(json([name=Name, value=ValueAtom, default=Default]));
      message_to_string(Error, Msg), reply_json(json([error= @true, msg=Msg, name=Name]))
    );
